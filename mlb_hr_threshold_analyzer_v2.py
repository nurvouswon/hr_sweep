...
st.set_page_config("2️⃣ MLB HR Predictor — Deep Ensemble + Weather Score [DEEP RESEARCH SOFT VOTING]", layout="wide")
st.title("2️⃣ MLB Home Run Predictor — Deep Ensemble + Weather Score [DEEP RESEARCH SOFT VOTING]")
...
# --- GAMEDAY OVERLAY LOGIC (tweak as you like for park/weather/lineup) ---
def gameday_overlay(row):
    # EXAMPLE: boost if "Coors Field", mild boost for "Wrigley", reduce for "Oracle Park", etc.
    # Tweak/expand as needed!
    ...
    if "oracle" in park:
        overlay *= 0.94
    # Example: additional weather (you can expand this)
    if "wind" in str(row.get('condition', '')).lower():
        overlay *= 1.02
    # You can add more logic: e.g., starter pitcher, lineup slot, etc.
    return overlay
...
if event_file is not None and today_file is not None:
    ...
    st.write("Training base models for soft-voting ensemble...")
    models_for_ensemble = [
        ('xgb', xgb.XGBClassifier(n_estimators=60, max_depth=5, learning_rate=0.08, use_label_encoder=False, eval_metric='logloss', n_jobs=1, verbosity=1, tree_method='hist')),
        ('lgb', lgb.LGBMClassifier(n_estimators=60, max_depth=5, learning_rate=0.08, n_jobs=1)),
        ('cat', cb.CatBoostClassifier(iterations=60, depth=5, learning_rate=0.09, verbose=0, thread_count=1)),
        ('rf', RandomForestClassifier(n_estimators=40, max_depth=7, n_jobs=1, verbose=1)),
        ('gb', GradientBoostingClassifier(n_estimators=40, max_depth=5, learning_rate=0.09, verbose=1)),
        ('lr', LogisticRegression(max_iter=400, solver='lbfgs', n_jobs=1, verbose=1)),
    ]
    ...
    for name, model in models_for_ensemble:
        try:
            ...
            fit_status.append(f"{name.upper()} OK")
            fitted_models.append((name, model))
        except Exception as e:
            st.warning(f"{name.upper()} failed: {e}")

    st.info("Model training status: " + ', '.join(fit_status))
    ...
    ensemble = VotingClassifier(estimators=fitted_models, voting='soft', n_jobs=1)
    ...
    # =========== GAMEDAY OVERLAY ===========
    st.write("Applying gameday overlays...")
    today_df["gameday_overlay_factor"] = today_df.apply(gameday_overlay, axis=1)
    today_df["final_hr_probability"] = today_df["hr_probability"] * today_df["gameday_overlay_factor"]
    ...
    st.markdown("### 🏆 **Today's HR Probability — Top 30 (Overlay Adjusted)**")
