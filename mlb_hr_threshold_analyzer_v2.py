...
def read_event_parquet(uploaded_file):
    return pd.read_parquet(uploaded_file)

@st.cache_data(show_spinner=True)
def read_today_csv(uploaded_file):
...
def fix_types(df):
    for col in df.columns:
        ...
        if df[col].dtype == 'O':
            try:
                df[col] = pd.to_numeric(df[col])  # robust, no errors='ignore'
            except Exception:
                pass
...
# ==== Streamlit UI ====
event_file = st.file_uploader("Upload Event-Level Parquet for Training (required)", type='parquet', key='eventparquet')
...
if event_file is not None and today_file is not None:
    try:
        with st.spinner("Loading & cleaning data..."):
            event_df = read_event_parquet(event_file)
            st.write("Loaded event-level parquet:", event_df.shape)
            today_df = read_today_csv(today_file)
            st.write("Loaded today CSV:", today_df.shape)
            event_df = dedup_columns(event_df)
            today_df = dedup_columns(today_df)
            event_df = fix_types(event_df)
            today_df = fix_types(today_df)
            st.write("After fix_types - event_df:", event_df.shape)
            st.write("After fix_types - today_df:", today_df.shape)
        ...
        st.write("Feature cols used:", feature_cols)

        X = clean_X(event_df[feature_cols])
        y = event_df[target_col]
        X_today = clean_X(today_df[feature_cols], train_cols=X.columns)
        st.write("Shapes: X", X.shape, "y", y.shape, "X_today", X_today.shape)

        # Train/test split
        X_train, X_val, y_train, y_val = train_test_split(
            X, y, test_size=0.15, random_state=42, stratify=y
        )
        st.write("Split shapes:", X_train.shape, X_val.shape)
        ...
        st.markdown("### 🏆 **Today's HR Probability Table**")
        ...
        st.success("Done! If you see this, the app ran to completion.")
    except Exception as e:
        st.error(f"Error: {e}")
        st.stop()
else:
    st.warning("Upload event-level Parquet and TODAY CSV to begin.")
