...
def get_batter_stats_multi(batter_name, windows):
    ...
    if not pid:
        for w in windows:
            for stat in ['BarrelRate','EV','HardHit','xBA','xSLG','wOBA']:
                out[f"B_{stat}_{w}"] = None
        return out
    for w in windows:
        ...
        try:
            ...
            hardhit = df[df['launch_speed'] >= 95].shape[0] / total if total > 0 else None
            xba = df['estimated_ba_using_speedangle'].mean() if 'estimated_ba_using_speedangle' in df and total > 0 else None
            xslg = df['estimated_slg_using_speedangle'].mean() if 'estimated_slg_using_speedangle' in df and total > 0 else None
            woba = df['estimated_woba_using_speedangle'].mean() if 'estimated_woba_using_speedangle' in df and total > 0 else None
            out[f"B_BarrelRate_{w}"] = round(barrel_rate,3)
            out[f"B_EV_{w}"] = round(ev,1) if ev else None
            out[f"B_HardHit_{w}"] = round(hardhit,3) if hardhit is not None else None
            out[f"B_xBA_{w}"] = round(xba,3) if xba is not None else None
            out[f"B_xSLG_{w}"] = round(xslg,3) if xslg is not None else None
            out[f"B_wOBA_{w}"] = round(woba,3) if woba is not None else None
        except Exception:
            for stat in ['BarrelRate','EV','HardHit','xBA','xSLG','wOBA']:
                out[f"B_{stat}_{w}"] = None
    return out
...
def get_pitcher_stats_multi(pitcher_name, windows):
    ...
    stats_list = [
        ...
        "xBAAllowed", "xSLGAllowed", "wOBAAllowed"
    ]
    if not pid:
        for w in windows:
    ...
    for w in windows:
        ...
        try:
            ...
            out[f"P_xBAAllowed_{w}"] = round(xba,3) if xba is not None else None
            out[f"P_xSLGAllowed_{w}"] = round(xslg,3) if xslg is not None else None
            out[f"P_wOBAAllowed_{w}"] = round(woba,3) if woba is not None else None
        except Exception:
            for stat in stats_list:
                out[f"P_{stat}_{w}"] = None
    return out
...
def norm_hardhit(x): return max(0, min(x, 1)) if pd.notnull(x) else 0
def norm_xba(x): return max(0, min((x - 0.2) / (0.35 - 0.2), 1)) if pd.notnull(x) else 0
def norm_xslg(x): return max(0, min((x - 0.35) / (0.7 - 0.35), 1)) if pd.notnull(x) else 0
def norm_woba(x): return max(0, min((x - 0.28) / (0.42 - 0.28), 1)) if pd.notnull(x) else 0
...
st.title("⚾️ MLB HR Matchup Leaderboard (All Pitcher/Batter Advanced Stats, 2025 Micro-Trends, Weather, Park, Handedness)")
...
if uploaded_file and xhr_file:
    ...
    st.write("Fetching Statcast, advanced metrics, weather (game time), park factor, and merging xHR (may take a few minutes)...")
    ...
    for idx, row in df_merged.iterrows():
        ...
        progress.progress((idx+1)/len(df_merged), text=f"{percent}%")
    ...
    # Extended HR Score including advanced stats
    def calc_hr_score(row):
        batter_score = (
            ...
            norm_hardhit(row.get('B_HardHit_14')) * 0.03 +
            norm_xba(row.get('B_xBA_14')) * 0.02 +
            norm_xslg(row.get('B_xSLG_14')) * 0.02 +
            norm_woba(row.get('B_wOBA_14')) * 0.02
        )
        pitcher_score = (
            norm_barrel(row.get('P_BarrelRateAllowed_14')) * 0.05 +
            norm_barrel(row.get('P_BarrelRateAllowed_7')) * 0.04 +
            ...
            norm_hardhit(row.get('P_HardHitRate_14')) * 0.02 +
            norm_xba(row.get('P_xBAAllowed_14')) * 0.01 +
            norm_xslg(row.get('P_xSLGAllowed_14')) * 0.01 +
            norm_woba(row.get('P_wOBAAllowed_14')) * 0.01
        )
    ...
    show_cols = [
        'Batter','Pitcher','BatterHandedness','PitcherHandedness','Park','City','Time','HR_Score','Reg_xHR',
        'B_BarrelRate_14','B_EV_14','B_HardHit_14','B_xBA_14','B_xSLG_14','B_wOBA_14',
        'ParkFactor','Temp','Wind','WindEffect',
        'P_BarrelRateAllowed_14','P_EVAllowed_14','P_HardHitRate_14',
        'P_xBAAllowed_14','P_xSLGAllowed_14','P_wOBAAllowed_14',
...
st.caption("""
- **All rolling batter and pitcher stats (3, 5, 7, 14 days) and all advanced stats (HardHit%, xBA, xSLG, wOBA) are included.**
- Weather, wind (at game time!), park factor, handedness, and xHR regression are all automated.
- Park and ballpark city shown in leaderboard.
